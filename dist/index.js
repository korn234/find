...
var DatabaseStorage = class {
  ...
  async getUserByNickname(nickname) {
    const [user] = await db.select().from(users2).where(eq2(users2.nickname, nickname));
    return user || void 0;
  }
  ...
  async updateUserSettings(userId, settingsData) {
    ...
    const [user] = await db.update(users2).set(updateData).where(eq2(users2.id, userId)).returning();
    return user;
  }
  async getPotentialMatches(userId, limit = 10) {
    const currentUser = await this.getUser(userId);
    if (!currentUser) return [];
    const matchedUserIds = await db.select({ userId: sql`CASE WHEN ${matches.userId1} = ${userId} THEN ${matches.userId2} ELSE ${matches.userId1} END` }).from(matches).where(or(eq2(matches.userId1, userId), eq2(matches.userId2, userId)));
    ...
    return await db.select().from(users2).where(
      and(
        excludeClause,
  ...
  async searchSchools(query, province) {
    ...
    return await db.select().from(schools).where(and(...conditions)).limit(20);
  }
  ...
  async getMatchById(matchId) {
    const [match] = await db.select().from(matches).where(eq2(matches.id, matchId));
    return match || void 0;
  }
  // Message operations
  ...
  async getMessages(matchId) {
    return await db.select().from(messages).where(eq2(messages.matchId, matchId)).orderBy(messages.createdAt);
  }
  ...
  async deleteMessages(matchId) {
    await db.delete(messages).where(eq2(messages.matchId, matchId));
  }
  async deleteAllUserMessages(userId) {
    const userMatches = await db.select({ id: matches.id }).from(matches).where(
  ...
  async deleteMatch(matchId) {
    await db.delete(matches).where(eq2(matches.id, matchId));
  }
};
...
async function hashPassword(plainPassword) {
  const saltRounds = 10;
  return await bcrypt2.hash(plainPassword, saltRounds);
}
...
var vite_config_default = defineConfig({
  plugins: [
    ...
    ...process.env.NODE_ENV !== "production" && process.env.REPL_ID !== void 0 ? [
      await import("vite-plugin-cartographer").then(
        (m) => m.cartographer()
      )
    ] : []
  ],
